...
let OH_Rank=1; //Number of OH category Seats
let HD_Rank=1; //Number of HD category Seats
let HB_Rank=1; //Number of HB category Seats
let SC_EX_Rank=1; //Number of SC EX-ARMY category Seats
let SC_F_Rank=1; //Number of SC FEMALE category Seats
let SC_Rank=1; // Number of SC COMMON TO ALL category Seats
let ST_EX_Rank=1; //Number of  ST EX-ARMY category Seats
let ST_F_Rank=1; //Number of ST FEMALE category Seats
let ST_Rank=1; //Number of ST COMMON TO ALL category Seats
let OBC_EX_Rank=1; //Number of OBC EX-ARMY category Seats
let OBC_F_Rank=1; //Number of OBC FEMALE category Seats
let OBC_Rank=1; //Number of OBC OPEN TO ALL category Seats
let UNR_EX_Rank=1; //Number of Unreserved EX-ARMY Seats
let UNR_F_Rank=1; // Number of Unreserved FEMALE Seats
let UNR_Rank=1; //Number of Unreserved OPEN TO ALL Seats
...
//detail.updateOne({_id:sorted_list[i].id},{$set:{All_Seat:sorted_list[i].category+'-'+Count_OH}});
	sorted_list[i].All_Seat=sorted_list[i].category+'-'+Count_OH;
	sorted_list[i].save();
	Count_=Count_OH+1;
	ALL_SEAT=ALL_SEAT-1;
	OH_Rank=OH_Rank-1;}
...
else if(sorted_list[i].ph==='HB' && sorted_list[i].relax==='Y' && HB_Rank>0)
{
	...
	sorted_list[i].All_Seat=sorted_list[i].category+'-'+Count_HB;
	sorted_list[i].save();
	Count_HB=Count_HB+1;
	ALL_SEAT=ALL_SEAT-1;
	HB_Rank=HB_Rank-1;}
...
else if(sorted_list[i].ph==='HD' && sorted_list[i].relax==='Y' && HD_Rank>0)
{
	...
	sorted_list[i].All_Seat=sorted_list[i].category+'-'+Count_HD;
	sorted_list[i].save();	
	Count_HD=Count_HD+1;
	ALL_SEAT=ALL_SEAT-1;
	HD_Rank=HD_Rank-1;}
...
else if(sorted_list[i].relax==='N' && sorted_list[i].ex==='EX' && UNR_EX_Rank>0)
{
...
else if(sorted_list[i].relax==='N' && sorted_list[i].gender==='F' && UNR_F_Rank>0)
{
